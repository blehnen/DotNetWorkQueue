### Unreleased
- **SQL Server transport: an ordinary send is now one round trip to the server rather than four.** The send opened a connection, inserted the body, read back the identity, inserted the meta row and committed - each a separate exchange with the server, and on anything but a local instance the network cost of those dominates the work itself. Body insert, `SCOPE_IDENTITY`, meta insert and the transaction now go as a single batch. Measured against a local instance, where the round trips are as cheap as they ever get: **9,601 us to 8,066 us per send (16%) and 29,298 B to 24,282 B (17%)**. The gain is larger the further away the server is (GitHub #231)
- The batch is all-or-nothing: `SET XACT_ABORT ON` with a server-side transaction, so a failed send leaves no orphaned body row, as the client-side transaction it replaces did. Covered by an integration test that forces a mid-batch failure (GitHub #231)
- Three shapes deliberately keep the four-round-trip path, because each needs to interleave work between the statements: a **scheduled job** (the "is this job already queued" check is a check-then-act that has to hold its lock across the insert), a send inside a **caller-supplied transaction** (the caller owns the transaction, so the batch cannot open its own), and a queue with the **status table** enabled (its insert needs the same `@CorrelationID` the meta insert binds, and a batch cannot carry the parameter twice). Behaviour is identical on all four; only the number of exchanges differs (GitHub #231)

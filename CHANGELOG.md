...
- LiteDb transport: a **shared** connection benefits most from the batch path above. Shared mode pays a fixed per-operation cost inside LiteDB — the engine is opened and closed under a cross-process mutex each time, which is what lets another process interleave — so a batch pays it once instead of once per message: 2,566 us a message sending one at a time against 68.6 us in a batch, 37x. Reusing the connection was measured and does **not** help there, unlike in direct mode

...
- Async consumers no longer hold a thread when a message fails. Counting the retry, writing the error count and moving the message to the error queue all ran synchronously, so the last blocking call on the async path is gone (GitHub #284)

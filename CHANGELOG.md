###0.6.6.3 2026-08-27
* SQLite transport only. No other package changes in this release.
* **SQLite true bulk-insert batch send**, back-ported from the 0.9.x line (#162), for consumers who cannot move off net4x. `Send(List<...>)` performs one multi-row body insert per chunk inside a single connection and transaction, instead of looping the single-message handler. Generated ids are recovered in caller input order. New `SqLiteMessageQueueTransportOptions.BatchSize` caps the chunk size; 0 (the default) uses the transport-computed safe maximum.
* Behavior change on the batch path only: it is whole-batch atomic. Any failure rolls back every message in the batch and each returned `IQueueOutputMessage` reports it. `Send(message)` in a loop keeps the previous per-message isolation. Scheduled-job messages are rejected on the batch path.
* **SQLite performance**, back-ported from the 0.9.x/0.10.0 line (#217, #218, #220, #221, #222, #223, #225). Connections and the statements SQLite compiled for their commands are reused between operations rather than rebuilt per message, the connection string is parsed once, and the dequeue SQL is built once per consumer.
* **WAL journal mode is now enabled by default** (`SqLiteMessageQueueTransportOptions.EnableWalMode`), back-ported because the pooling change above is worth almost nothing without it -- pooling avoids the checkpoint SQLite performs when the last connection to a WAL database closes, and there is no such checkpoint in rollback-journal mode. Set it to false to keep the previous journal mode.
...
* Breaking for anyone implementing `IDbFactory` themselves: it gains `CreateCommand(IDbConnection, string)`. On the 0.9.x line this was added as a default interface method, which net4x and netstandard2.0 do not support, so here it is a plain declaration. `DbFactory` is the only implementation in the library.
* `DotNetWorkQueue.Transport.SQLite` was last published at 0.6.6 -- it was not part of the 0.6.6.1 or 0.6.6.2 releases. This release resumes it at 0.6.6.3.

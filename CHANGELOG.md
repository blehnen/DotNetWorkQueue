### Unreleased
- **SQLite transport: the dequeue SQL is built once and cached, cutting a dequeue's allocation by 91%.** Generating the script was 6,848 B of the 7,496 B a dequeue allocated. It is now built on first use for a given set of routes and caller clause and kept. Measured on net10 against an empty queue: 7,170 ns and 7,496 B → 5,800 ns and 648 B; against the shape before any of the command work, 29,854 ns and 22,144 B → 5,800 ns and 648 B, 5.1x on time and 34x on allocation. The caller clause and parameters are still read on every dequeue, so the factory forms of `SetUserParametersAndClause` behave exactly as before — the clause is part of the cache key, and parameter values are bound rather than written into the SQL
- **SQLite transport: send commands are reused too, so their statements are compiled once instead of per message.** The same change as the dequeue path, applied to the body insert, the generated-id lookup, the meta data insert and the status insert, in the single and batch handlers alike. Measured on net10, a send went from 93,441 ns and 23,370 B to 80,213 ns and 20,674 B — 14% faster, 12% less allocated. Batch sends benefit most: a chunk previously recompiled the meta data and status inserts once per message. Reuse depends on the statement text being identical between sends. Caller-defined columns are named in that text — on the meta data insert when `AdditionalColumnsOnMetaData` is on, on the status insert when it is off — so a caller that varies its column names from message to message varies the statement too, and that statement falls back to the previous behaviour. Varying only the *values* is unaffected, which is the ordinary case
- **SQLite transport: dequeue commands are reused, so their statements are compiled once instead of per message.** System.Data.SQLite compiles a command's statements on first execution and keeps them on the command object, so a command created per operation recompiles every time — and the dequeue script is long and has several statements. Measured on net10 against an empty queue, a dequeue went from 27,760 ns and 22,144 B to 6,021 ns and 7,496 B, a 4.6x improvement. Compiled commands live on the pooled connection and are released when it is, so disposing the consumer still releases the database file. `IDbFactory` gains `CreateCommand(IDbConnection, string)` as a default interface method, so existing implementations keep working unchanged
- **SQLite transport: the connection string is parsed once and the result cached.** Parsing means constructing a `SQLiteConnectionStringBuilder`, measured at 7.2 us and 20.2 KB on net10 — a third of everything a send allocated. A send parsed twice: once for the database existence check and once when creating a connection. Measured end to end, a single send went from ~101 us to ~82 us and from 63.2 KB to 22.8 KB — a 19% latency and 64% allocation reduction. `DatabaseExists` still checks the file system on every call, so a database deleted underneath a live queue is still reported correctly
- **SQLite transport: connections are now reused between operations, and kept open.** Constructing a `SQLiteConnection` costs roughly 400 us — far more than opening the database, which costs about 63 us — so the provider's own pool, which reuses the *native handle*, recovers little on its own. Reusing the managed object is what matters, and only while it stays open. Measured for one insert on net10: a new connection per operation 695.8 us, an object pool closing on return 188.3 us, an object pool leaving the connection open 14.8 us. End to end a single send went from ~0.89 ms to ~0.36 ms, and from ~8.5 ms to ~0.36 ms measured against the pre-pooling baseline
- ⚠️ **Disposing the producer or consumer is what releases the SQLite database file.** Pooled connections are held open for the lifetime of the queue that created them, so a caller that deletes a database file while a queue is still alive will find it locked. Queues are `IDisposable` and the usual `using` pattern already gives the right ordering; `SqLiteMessageQueueCreation.RemoveQueue()` cannot release connections held by a *live* queue in another container, because they are open rather than idle and so are not affected by `SQLiteConnection.ClearAllPools()`
- SQLite transport: in-memory databases and hold connections are never pooled — the hold connection already keeps an in-memory database alive, and with `cache=shared` an extra open connection would keep one alive past the point the caller disposed of it
- **SQLite transport: connection pooling is now enabled by default.** `System.Data.SQLite` defaults `Pooling` to `false`, unlike the SQL Server and PostgreSQL providers, and the transport opens a connection per operation. In WAL journal mode — the transport default since `EnableWalMode` — SQLite checkpoints the write-ahead log when the *last* connection to a database closes, so an unpooled connection made every send pay a checkpoint. Measured single-threaded on net10: **8.54 ms → 0.92 ms per send, a 9.2× improvement**. The same change against a database in rollback-journal mode made no difference (5.87 ms vs 5.86 ms), which is what identifies the close-checkpoint rather than connection-open cost as the thing pooling avoids here
- ⚠️ **Breaking behavior change for callers that delete SQLite database files.** A pooled connection keeps the database file handle open, so `File.Delete` on a database the transport has touched will now fail where it previously succeeded. `SqLiteMessageQueueCreation.RemoveQueue()` calls `SQLiteConnection.ClearAllPools()` on your behalf, so the usual remove-then-delete sequence is safe. Callers that delete a database file **without** going through `RemoveQueue()` — test fixtures especially — must call `SQLiteConnection.ClearAllPools()` first, and should delete the `-wal` and `-shm` files alongside the database. Opt out entirely by putting `Pooling=False` in your connection string; an explicit `Pooling=True` or `Pooling=False` is always respected
- SQLite transport: in-memory databases are never pooled — the hold connection already keeps them alive, and with `cache=shared` a pooled connection would keep a database alive past the point the caller disposed of it

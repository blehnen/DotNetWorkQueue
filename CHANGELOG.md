### Unreleased
- **SQLite transport: the dequeue temp table is reused instead of created per message.** Each dequeue staged its candidate row in a uniquely named `CREATE TEMP TABLE`. A temp table lives until its connection closes, and now that connections are pooled and held open for the life of the queue, every committed dequeue left one behind. Measured on net10, the create alone cost 46.7 us at 100 messages and 346.1 us at 5,000 and was still climbing, with all 5,000 tables resident; it is now flat at ~5 us with a single table. The name is derived from the queue and the statement is `CREATE TEMP TABLE IF NOT EXISTS`, so it is a no-op after the first dequeue on a connection. Introduced by the connection reuse above and fixed before release — no published version is affected

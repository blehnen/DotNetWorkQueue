### Unreleased
- **LiteDb transport: queues no longer serialize against each other.** The send and receive paths each held a process-wide `static` lock, so every LiteDb queue in a process waited on every other one — including queues using entirely separate database files. Those locks are now scoped to the database they protect, and the send lock is taken only for scheduled jobs, which is the check-then-act it existed for. Measured on two queues across four threads: 200 sends went from 40.3 ms to 22.4 ms, and from slower than single-threaded to faster (GitHub #234)

...
- The netstandard2.0 build has no PostgreSQL transport and no `AesMessageInterceptor`. Npgsql's last netstandard2.0 line goes out of support in November 2026, and AES-GCM does not exist on that framework (GitHub #252)
- ⚠️ `IDbFactory.CreateCommand(DbConnection, string)` no longer has a default implementation. Only affects a custom SQLite `IDbFactory`; the one shipped here already declared it (GitHub #252)

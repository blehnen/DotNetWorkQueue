...
- Fix: with `EnableHoldTransactionUntilMessageCommitted`, moving a failed message to the error queue is now part of the same transaction as removing it from the queue. A failure partway left the message in the error queue *and* back in the queue, so it could be processed twice (GitHub #300)

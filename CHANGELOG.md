### Unreleased
- PostgreSQL and SQL Server: queues using `EnableDelayedProcessing` or `EnableMessageExpiration` now get the cached send statement they were excluded from (GitHub #255)
- SQL Server: a queue with varying delays no longer makes the server compile a new plan for each delay value (GitHub #255)
...
- PostgreSQL transport: the send statement is now cached per table and option shape, which the transport did not do at all - SQL Server gained it earlier in #231 (GitHub #232)

### Unreleased
- ⚠️ `TripleDesMessageInterceptor` and `TripleDesMessageInterceptorConfiguration` are gone, deprecated since 0.9.43. Drain or re-encrypt any 3DES messages before upgrading: nothing can read them afterwards. Use `AesMessageInterceptor` (GitHub #183)
- ⚠️ The dashboard's `Interceptors:TripleDes` setting is gone with it. A configuration that still has one is refused at startup rather than ignored, which would have left the dashboard unable to decrypt (GitHub #183)
...
- ⚠️ Fix: a failing message now gets every attempt it was configured for on SQL Server, PostgreSQL and SQLite. A database fault could swallow an attempt the message never made. Two workers failing the same message at once now count one failure between them instead of two (GitHub #350)
- Fix: retry delays set through `RetryDelayBehavior` work again on SQL Server, PostgreSQL and SQLite. A failing message used to come straight back instead of waiting for the configured back-off (GitHub #352)
- Fix: with delayed processing on, a rolled-back message goes to the back of the queue again on SQL Server, PostgreSQL and SQLite. It used to keep its place at the front, so a consumer that cancelled the same message over and over re-read it in a tight loop and never reached the rest (GitHub #352)
- ⚠️ SQL Server queue names are limited to 101 characters rather than 128. Anything longer failed at creation on an over-long index name. Names of 102 to roughly 113 worked only while history was off, and would have broken the first time someone enabled it (GitHub #344)
...
- ⚠️ Fix: two failures arriving at once each added an error row, so a message got more retries than configured and could loop instead of reaching the error queue. Existing queues keep the old behaviour until the 0.12.0 upgrade script adds the index (GitHub #299)
...
- ⚠️ Custom transports and consumers must add async members to the receive, commit, rollback, error and history interfaces. GitHub #284 lists every member; built-in transports are unaffected (GitHub #284)
...
- Async consumers no longer hold a thread finishing a message while a heartbeat is mid-update. This was the last synchronous call on the async receive path (GitHub #284)
- A heartbeat no longer occupies a thread-pool thread for its transport round trip, on any transport (GitHub #284)
- Fix: tracing a heartbeat threw when the transport returned no status, which Redis does for a message with no id. The failure was reported as a heartbeat error and cancelled the message's token (GitHub #284)
...
- ⚠️ A consumer is now cancelled when no heartbeat has been recorded for its message within the configured window. It was carrying on with a message the monitor may already have handed to another worker, so the same message could be completed twice (GitHub #294)
- ⚠️ Fix: removing a message that is not there now reports `NotFound` rather than `Removed` on SQL Server, PostgreSQL and SQLite. Counts from `ClearExpiredMessages` and `ClearErrorMessages`, and the dashboard delete, were reporting removals that did not happen (GitHub #292)
- Fix: a failure while moving a message to the error queue could leave it in the error queue *and* back in the queue, where it would be processed a second time. Affects queues using `EnableHoldTransactionUntilMessageCommitted` (GitHub #300)
- Redis: recovering from a `NOSCRIPT` after a restart, failover or `SCRIPT FLUSH` no longer blocks a thread while it reloads the script on the async path (GitHub #290)

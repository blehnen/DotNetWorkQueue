### Unreleased
- **Core library: a send allocates less than half of what it did, on every transport.** `AdditionalMessageData` — one is built for every message sent — created four collections in its constructor that most messages never put anything in, the largest a `ConcurrentDictionary` whose lock array is sized from the processor count. They are now created on first use: 1,832 B and 574 ns per message down to 72 B and 32 ns. `Message` likewise built a second dictionary for internal headers that most sends never touch, and `IAdditionalMessageData.Headers` built a fresh `ReadOnlyDictionary` wrapper on every read while the send path read it twice. Measured on the transport-independent producer pipeline: **3.02 KB and 2,990 ns per message down to 1.13 KB and 2,280 ns**; a SQLite send 17.64 KB -> 15.59 KB and a 100-message SQLite batch 1,562 KB -> 1,361 KB, with Gen1 collections down from 21.5 to 5.9 per thousand sends (GitHub #235)

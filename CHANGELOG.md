### Unreleased
- **`System.Text.Json` is available as an opt-in message serializer.** Newtonsoft remains the default, because callers can queue any POCO and Newtonsoft handles more shapes without annotation. For callers who own their message types, `SystemTextJsonSerializer` allocates 752 B against 4,464 B to serialize a 256-byte message and 1,272 B against 4,552 B to read one back - roughly 7 KB less garbage per message round trip, on every transport. No new dependency; `System.Text.Json` is in-box. See [docs/serializers.md](docs/serializers.md) (GitHub #236)

### Unreleased
- ⚠️ Fix: a failing message is retried the number of times it was configured for on SQL Server, PostgreSQL and SQLite. The error count was incremented rather than set, and the write is retried, so a transient fault after the server had already committed it counted one failure twice and the message reached the error queue with attempts still owed. Two workers failing the same message at the same instant now count as one failure rather than two, which costs an extra attempt rather than losing one (GitHub #350)
- Fix: retry delays configured through `RetryDelayBehavior` are applied again on SQL Server, PostgreSQL and SQLite. A failing message was retried immediately instead of after the configured back-off — measured at 0.04s against a configured 8s (GitHub #352)

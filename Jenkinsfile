...
def useInternalNugetMirror() {
    sh '''
        ...
        MARKER="generated by the Jenkinsfile - do not commit"

        if [ -z "${NUGET_SOURCE:-}" ]; then
            # Agents keep state between builds: a checkout takes 4s and restoring
            # 146 packages takes 6s, neither of which is possible cold. So a config
            # written when a mirror was configured would outlive it, and its
            # <clear /> would pin every later restore in this workspace to a source
            # that no longer exists. Remove it - but only if this helper wrote it,
            # so a NuGet.Config the repository gains later is left alone.
            if [ -f "$CONFIG" ] && grep -q "$MARKER" "$CONFIG"; then
                rm -f "$CONFIG"
                echo "Removed a stale generated NuGet.Config; restores will use nuget.org"
            fi
            exit 0
        fi

...
| finding | evidence |
|---|---|
| It pays, modestly, and mostly on batches | Single send 5.938 ms against 5.565 ms, about 6%; a batch of 100 goes 37.386 ms to 32.460 ms, about 13%. Allocation is unchanged either way. The batch figure is solid; the single-send one is roughly one error bar wide on the `off` rung and should be read as suggestive rather than firm |

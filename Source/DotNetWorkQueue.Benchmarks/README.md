...
| The delay and the expiration ride as parameters | `DATEADD(ms, @QueueProcessOffset, GetUTCDate())`. They were literals - `DATEADD(ms,12345,...)` - which kept the send-side cache to the invariant shape and made SQL Server compile a fresh plan per distinct delay value. Only the offset is parameterised, so the server's clock still sets the base time. Fixed in #255 |
...
| A delayed-processing queue is cached like any other | `AddBuiltInColumnValues` used to inline `currentDateTime.Ticks` whenever `EnableDelayedProcessing` was on - **even for a message carrying no delay** - so every send produced a different statement, the cache never hit and PostgreSQL re-planned each send. Both values are bigint .NET ticks, so unlike SQL Server there is no server-side expression to use; they are bound as parameters instead. Fixed in #255 |

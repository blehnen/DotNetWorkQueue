...
namespace DotNetWorkQueue.IntegrationTests.Shared.Producer
{
    public class ProducerAsyncShared
    {
        public async Task RunTest<TTransportInit, TMessage>(string queueName,
            ...
            Action<string, QueueProducerConfiguration, long> verify, bool sendViaBatch)
        ...
        private async Task RunProducer<TMessage>(
          IProducerQueue
              <TMessage> queue,
                string queueName,
                long messageCount,
            ...
            where TMessage : class
        {
            await RunProducerInternal(queue, messageCount, generateData, sendViaBatch).ConfigureAwait(false);
            LoggerShared.CheckForErrors(queueName);
            verify(queueName, queue.Configuration, messageCount);
        }

        private async Task RunProducerInternal<TMessage>(
           IProducerQueue

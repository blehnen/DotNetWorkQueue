    ...
    public class ClaimIsKnownBeforeTheFirstBeatTests
    {
        ...
        public void TheClaimIsKnownWhenTheHandlerRuns()
        {
                ...
                serviceRegister => serviceRegister.Register(() => logProvider, LifeStyles.Singleton)))
            {
                ...
                try
                {
                    //LiteDb's options are read-only and its heartbeat is always on; it stores a BSON date
                    //at millisecond precision, so the claim it publishes is truncated to match what is
                    //actually written - see #328
                    Assert.IsTrue(oCreation.CreateQueue().Success);

                    new ProducerShared().RunTest<LiteDbMessageQueueInit, FakeMessage>(queueConnection, false, 1,

...
namespace DotNetWorkQueue.Transport.LiteDb.Basic.QueryHandler
{
    internal class GetDashboardConfigurationQueryHandlerAsync : IQueryHandlerAsync<GetDashboardConfigurationQuery, byte[]>
    {
        ...
        public Task<byte[]> HandleAsync(GetDashboardConfigurationQuery query)
        {
            ...
            {
                var col = db.Database.GetCollection<Schema.ConfigurationTable>(_tableNameHelper.ConfigurationName);
                var record = col.Query().FirstOrDefault();
                return Task.FromResult(record?.Configuration);
            }
        }
    }
}

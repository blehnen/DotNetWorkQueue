...
namespace DotNetWorkQueue.Transport.Memory.Tests.Basic.QueryHandler
{
    public class GetDashboardConfigurationQueryHandlerAsyncTests
    {
        ...
        {
            Assert.NotNull(new GetDashboardConfigurationQueryHandlerAsync(Substitute.For<IDataStorage>()));
        }

        [Fact]
        public void Create_NullDataStorage_Throws()
        {
            Assert.Throws<ArgumentNullException>(() => new GetDashboardConfigurationQueryHandlerAsync(null));
        }
    }
}

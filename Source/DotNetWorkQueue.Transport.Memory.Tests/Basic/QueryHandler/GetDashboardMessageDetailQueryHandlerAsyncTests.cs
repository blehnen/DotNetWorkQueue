...
namespace DotNetWorkQueue.Transport.Memory.Tests.Basic.QueryHandler
{
    public class GetDashboardMessageDetailQueryHandlerAsyncTests
    {
        ...
        {
            Assert.NotNull(new GetDashboardMessageDetailQueryHandlerAsync(Substitute.For<IDataStorage>()));
        }

        [Fact]
        public void Create_NullDataStorage_Throws()
        {
            Assert.Throws<ArgumentNullException>(() => new GetDashboardMessageDetailQueryHandlerAsync(null));
        }
    }
}

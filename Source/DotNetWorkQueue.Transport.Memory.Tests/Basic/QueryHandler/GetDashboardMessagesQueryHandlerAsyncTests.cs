...
namespace DotNetWorkQueue.Transport.Memory.Tests.Basic.QueryHandler
{
    ...
    public class GetDashboardMessagesQueryHandlerAsyncTests
    {
        ...
        {
            Assert.IsNotNull(new GetDashboardMessagesQueryHandlerAsync(Substitute.For<IDataStorage>()));
        }

        [TestMethod]
        public void Create_NullDataStorage_Throws()
        {
            Assert.ThrowsExactly<ArgumentNullException>(() => new GetDashboardMessagesQueryHandlerAsync(null));
        }
    }
}

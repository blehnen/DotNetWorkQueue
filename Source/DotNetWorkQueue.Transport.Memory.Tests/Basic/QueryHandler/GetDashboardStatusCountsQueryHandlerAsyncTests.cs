...
namespace DotNetWorkQueue.Transport.Memory.Tests.Basic.QueryHandler
{
    ...
    public class GetDashboardStatusCountsQueryHandlerAsyncTests
    {
        ...
        {
            Assert.IsNotNull(new GetDashboardStatusCountsQueryHandlerAsync(Substitute.For<IDataStorage>()));
        }

        [TestMethod]
        public void Create_NullDataStorage_Throws()
        {
            Assert.ThrowsExactly<ArgumentNullException>(() => new GetDashboardStatusCountsQueryHandlerAsync(null));
        }
    }
}

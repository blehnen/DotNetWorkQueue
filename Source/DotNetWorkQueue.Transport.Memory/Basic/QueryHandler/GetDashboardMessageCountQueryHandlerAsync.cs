    ...
    internal class GetDashboardMessageCountQueryHandlerAsync : IQueryHandlerAsync<GetDashboardMessageCountQuery, long>
    {
        ...
        public Task<long> HandleAsync(GetDashboardMessageCountQuery query)
        {
            if (query.StatusFilter == 0)
                return Task.FromResult(_dataStorage.RecordCount);

            if (query.StatusFilter == 1)
                return Task.FromResult(_dataStorage.WorkingRecordCount);

            if (query.StatusFilter == 2)
                return Task.FromResult(0L);

            // null — all
            return Task.FromResult(_dataStorage.RecordCount + _dataStorage.WorkingRecordCount);
        }
    }
}

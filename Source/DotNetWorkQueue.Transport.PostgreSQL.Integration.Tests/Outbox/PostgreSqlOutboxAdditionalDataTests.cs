...
namespace DotNetWorkQueue.Transport.PostgreSQL.Integration.Tests.Outbox
{
    ...
    /// Send path on PostgreSQL. Verification uses a direct SQL query against the queue's MetaData
    /// table rather than a live consumer dequeue, avoiding consumer lifecycle complexity.
    ///
    /// Assertions:
    ...
    public class PostgreSqlOutboxAdditionalDataTests : PostgreSqlOutboxIntegrationTestBase
    {
        ...
        /// single persisted row equals <paramref name="expected"/>. PG MetaData stores
        /// priority as <c>smallint</c>; cast to <see cref="byte"/> matches the SqlServer
        /// extractor in <c>VerifyQueueData.VerifyPriority()</c>.
        /// </summary>

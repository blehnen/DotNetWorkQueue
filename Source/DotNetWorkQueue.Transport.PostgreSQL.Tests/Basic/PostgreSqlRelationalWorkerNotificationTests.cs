...
{
    /// <summary>
    /// Contract + behavior tests for <see cref="PostgreSqlRelationalWorkerNotification"/>.
    /// Pairs with <see cref="PostgreSqlRelationalWorkerNotificationRegistrationTests"/> for the
    /// option-driven SimpleInjector smoke tests.
    /// </summary>
    [TestClass]
    public class PostgreSqlRelationalWorkerNotificationTests
    {
        private static PostgreSqlRelationalWorkerNotification CreateSubject(
            ...
            out ActivitySource tracer)
        {
            ...
            return new PostgreSqlRelationalWorkerNotification(headers, cancelWork, configuration, log, metrics, tracer);
        }
        ...
        {
            var subject = CreateSubject(out _, out _, out _, out _, out _, out _);
            var holder = Substitute.For<IConnectionHolder<NpgsqlConnection, NpgsqlTransaction, NpgsqlCommand>>();
            holder.Transaction.Returns((NpgsqlTransaction)null);
        ...
        {
            // NpgsqlTransaction is sealed → NSubstitute cannot proxy it, so the non-null-return path of
            ...
            // property round-trips. Full non-null-return coverage lands in Phase 7 PostgreSQL
            // integration tests where a real NpgsqlTransaction is available.
            var subject = CreateSubject(out _, out _, out _, out _, out _, out _);
            var holder = Substitute.For<IConnectionHolder<NpgsqlConnection, NpgsqlTransaction, NpgsqlCommand>>();
        ...
        public void Cast_To_IRelationalWorkerNotification_Succeeds()
        {
            ...
            Assert.IsTrue(subject is IRelationalWorkerNotification, "PostgreSqlRelationalWorkerNotification must implement IRelationalWorkerNotification.");

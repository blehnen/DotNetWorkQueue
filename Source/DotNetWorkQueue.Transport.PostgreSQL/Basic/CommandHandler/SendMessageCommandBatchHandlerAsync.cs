...
namespace DotNetWorkQueue.Transport.PostgreSQL.Basic.CommandHandler
{
    ...
    /// messages as a true bulk insert in one transaction; whole-batch atomic. Uses real async IO
    /// (<c>ExecuteReaderAsync</c>/<c>ExecuteNonQueryAsync</c>), matching the PostgreSQL
    /// single-message async handler.
    /// </summary>
    internal class SendMessageCommandBatchHandlerAsync : ICommandHandlerWithOutputAsync<SendMessageCommandBatch, QueueOutputMessages>
    {
        ...
        private async Task<int> ProcessChunkAsync(NpgsqlConnection connection, NpgsqlTransaction trans,

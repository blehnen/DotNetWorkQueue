    ...
    public class PostgreSqlCommandStringCache : CommandStringCache
    {
        ...
        protected override void BuildCommands()
        {
            ...
            CommandCache.Add(CommandStringTypes.DashboardRequeueErrorMessage,
                $"INSERT INTO {TableNameHelper.MetaDataName} (QueueID, CorrelationID, QueuedDateTime, Status) SELECT @QueueID, @CorrelationID, now() at time zone 'utc', {Convert.ToInt32(QueueStatuses.Waiting)} FROM {TableNameHelper.MetaDataErrorsName} WHERE QueueID = @QueueID LIMIT 1 ON CONFLICT(QueueID) DO UPDATE SET Status = {Convert.ToInt32(QueueStatuses.Waiting)}, HeartBeat = NULL");

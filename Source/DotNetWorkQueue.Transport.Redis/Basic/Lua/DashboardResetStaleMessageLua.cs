...
{
    /// <summary>
    /// Resets a single stale (processing/heartbeat-expired) message back to pending
    /// </summary>
    internal class DashboardResetStaleMessageLua : BaseLua
    {
        /// <inheritdoc />
        public DashboardResetStaleMessageLua(IRedisConnection connection, RedisNames redisNames)
            : base(connection, redisNames)
        {
            Script = @"local score = redis.call('zscore', @workingkey, @uuid)
                       if not score then
                           return 0
                       end
        ...
        /// <summary>
        /// Resets a stale working message back to pending.
        /// </summary>
        /// <param name="messageId">The message identifier.</param>
        /// <returns>1 if the message was reset, 0 if not found in working set.</returns>

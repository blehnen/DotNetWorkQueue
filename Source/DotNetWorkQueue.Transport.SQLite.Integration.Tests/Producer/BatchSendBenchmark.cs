...
namespace DotNetWorkQueue.Transport.SQLite.Integration.Tests.Producer
{
    ...
    /// captures the headline throughput win and logs it. SQLite is where the per-commit fsync win
    /// is largest, so the logged speedup is expected to be the most dramatic of the three
    /// transports. The pass/fail check is correctness (both paths write every row); the wall-clock
    /// comparison is logged as informational only, since a hard timing gate is flaky on shared CI
    ...
    public class BatchSendBenchmark
    {
        ...
        public BatchSendBenchmark(ITestOutputHelper output)
        {
            _output = output;
        }
        ...
        private static long RunScenario(bool sendViaBatch, int count)
        {
            using (var connectionInfo = new IntegrationConnectionInfo(false))
            {
                ...
                {
                    Assert.True(oCreation.CreateQueue().Success);
                    try
                    {
                        ...
                        using (var producer = qc.CreateProducer<FakeMessage>(queueConnection))
                        {
                            ...
                            else
                            {
                                for (var i = 0; i < count; i++)
                                {
                                    ...
                                    Assert.Null(output.SendingException);
                                }
                            }
                            sw.Stop();

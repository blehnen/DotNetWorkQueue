                ...
                using (var oCreation = queueCreator.GetQueueCreation<SqLiteMessageQueueCreation>(queueConnection))
                {
                    ...
                    try
                    {
                        ...
                        using (var producer = qc.CreateProducer<FakeMessage>(queueConnection))
                        {
                            ...
                            Assert.False(output.HasErrors, "batch send reported errors");

                            var ids = output.Select(m => Convert.ToInt64(m.SentMessage.MessageId.Id.Value)).ToList();
                            Assert.Equal(count, ids.Count);
                            ids.Distinct().Count().Should().Be(count, "generated ids are not unique");
                            ...
                            for (var i = 0; i < count; i++)
                            {
                                ...
                                orderId.Should().Be(i, $"id at result position {i} maps to the wrong input message");
                            }
                        }
                    }
                    finally
                ...
                using (var oCreation = queueCreator.GetQueueCreation<SqLiteMessageQueueCreation>(queueConnection))
                {
                    ...
                    try
                    {
                        ...
                        using (var producer = qc.CreateProducer<FakeMessage>(queueConnection))
                        {
                            ...
                            var output = producer.Send(messages);
                            Assert.True(output.HasErrors, "expected the batch to fail");
                            Assert.True(output.All(m => m.SendingException != null),
                                "every result should report the batch failure");
                        }

                        // Whole-batch atomic: no body rows were committed.
                        ScalarInt(connectionInfo.ConnectionString,
                                $"select count(*) from {nameHelper.QueueName}")
                            .Should().Be(0, "body rows were committed despite a mid-batch failure");
                    }
                    finally
                    {
                        oCreation.RemoveQueue();

...
namespace DotNetWorkQueue.Transport.SQLite.Integration.Tests.UserDequeue
{
    ...
    public class UserDequeueFromFactory
    {
            ...
            bool inMemoryDb, int valueCount, bool enableChaos)
        {
            ...
            _clauseCalls.Count.Should().Be(valueCount, "every consumer should have used its clause factory");
            _parameterCalls.Count.Should().Be(valueCount, "every consumer should have used its parameter factory");

            foreach (var calls in _clauseCalls)
                Assert.True(calls.Value > 1, $"the clause factory for order {calls.Key} was called {calls.Value} time(s); it must be consulted per dequeue");

            foreach (var calls in _parameterCalls)
                Assert.True(calls.Value > 1, $"the parameter factory for order {calls.Key} was called {calls.Value} time(s); it must be consulted per dequeue");
        }

...
{
    /// <summary>
    /// Pins the chunk-size selection the SQLite batch handler relies on: a configured
    /// <c>BatchSize</c> ceiling is honored when below the transport safe-max and clamped to the
    /// safe-max when above it, so a configured value can never overflow the SQLite parameter
    /// budget. (The handler builds exactly this <see cref="SendBatchSize"/> from

...
namespace DotNetWorkQueue.Transport.SQLite.Tests.Basic
{
    ...
    public class GetFileNameFromConnectionStringTests
    {
        ...
        public void FileDatabase_ReturnsTheFileName()
        {
            ...
            Assert.False(result.IsInMemory);
            Assert.Equal(@"c:\test\temp.db3", result.FileName);
        ...
        public void NullOrEmpty_IsNotValid()
        {
            ...
            Assert.False(Parser.GetFileName(null).IsValid);
            Assert.False(Parser.GetFileName(string.Empty).IsValid);
        }
        ...
        public void RepeatedCalls_ReturnTheSameAnswer()
        {
            ...
            Assert.Equal(first.IsInMemory, second.IsInMemory);
        }
        ...
        public void DifferentConnectionStrings_DoNotShareAnAnswer()
        {
            ...
            Assert.Equal(@"c:\test\one.db3", first.FileName);
            Assert.Equal(@"c:\test\two.db3", second.FileName);
        }
        ...
        public void CasingOfTheConnectionString_IsNotTreatedAsTheSameEntry()
        {
            ...
            Assert.Equal(@"c:\test\case.db3", lower.FileName);
            Assert.Equal(@"c:\test\case.db3", upper.FileName);
        }
        ...
        public void StaysCorrectBeyondTheCacheCap()
        {
            ...
            for (var i = 0; i < 300; i++)
            {
                ...
                Assert.Equal($@"c:\test\cap{i}.db3", Parser.GetFileName(connectionString).FileName);
            }
        }
        ...
        public void AnExistingFile_IsFoundThroughDatabaseExists()
        {
            ...
            try
            {
                Assert.True(exists.Exists(connectionString));

                File.Delete(fileName);

                Assert.False(exists.Exists(connectionString), "a deleted database must not be reported as existing because its parse was cached");
            }
            finally
            {

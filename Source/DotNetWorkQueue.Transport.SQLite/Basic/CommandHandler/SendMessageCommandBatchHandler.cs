...
namespace DotNetWorkQueue.Transport.SQLite.Basic.CommandHandler
{
    ...
    /// transaction spanning all chunks, a multi-row <c>INSERT … RETURNING QueueID</c> body insert
    /// per chunk (generated ids recovered in input order by ascending sort), and per-message
    /// meta/status inserts. The whole batch is atomic — any failure rolls back every row and each
    /// returned result reports the failure.
    /// </summary>
    /// <remarks>
    ...
    internal class SendMessageCommandBatchHandler : ICommandHandlerWithOutput<SendMessageCommandBatch, QueueOutputMessages>
    {
            ...
            IQueueOutputMessage[] results, ref int globalIndex)
        {
            ...
            //    (QueueID is INTEGER PRIMARY KEY AUTOINCREMENT, so the ids are assigned
            //    monotonically in VALUES order within this single write transaction).

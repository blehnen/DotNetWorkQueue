...
namespace DotNetWorkQueue.Transport.SQLite.Basic
{
    ...
    /// <see cref="ConnectionState"/> is set post-construction by the receive path
    /// pattern-matching the resolved <see cref="IWorkerNotification"/> as this type
    /// and assigning the <see cref="SqLiteConnectionState"/> it stored on the
    /// <see cref="IMessageContext"/>. The same context.WorkerNotification reference
    /// flows from <c>MessageContext</c> ctor (one factory.Create() per message)
    /// through to <c>ProcessMessage.Handle</c>, so the property set in the receive
    /// path is visible to the user handler reading <see cref="Transaction"/>.
    /// </para>
    /// </remarks>
    internal class SqLiteRelationalWorkerNotification : WorkerNotification, IRelationalWorkerNotification
    {
        ...
        /// post-construction by the receive path (pattern-match on
        /// <see cref="IMessageContext.WorkerNotification"/>) before the user handler
        /// runs. Null only between construction and the receive-path injection call.
        /// </summary>
        ...
        /// outside an active dequeue (i.e., before the receive path installs state).
        /// </remarks>

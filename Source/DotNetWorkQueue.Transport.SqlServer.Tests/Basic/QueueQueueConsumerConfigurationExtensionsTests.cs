...
namespace DotNetWorkQueue.Transport.SqlServer.Tests.Basic
{
    ...
    public class QueueQueueConsumerConfigurationExtensionsTests
    {
        ...
        {
            var config = CreateConfig();
            var parameters = new List<SqlParameter> { new SqlParameter("@p1", 1) };
        ...
        {
            var config = CreateConfig();
            var parameters1 = new List<SqlParameter> { new SqlParameter("@p1", 1) };
            config.SetUserParameters(parameters1);

            var parameters2 = new List<SqlParameter> { new SqlParameter("@p2", 2), new SqlParameter("@p3", 3) };
        ...
        {
            var config = CreateConfig();
            config.AddUserParameter(new SqlParameter("@p1", 1));
        ...
        {
            var config = CreateConfig();
            var parameters = new List<SqlParameter> { new SqlParameter("@p1", 42) };
            Func<List<SqlParameter>> paramFactory = () => parameters;
        ...
        public void SetUserParametersAndClause_Factory_Overrides_StaticValues()
        {
            ...
            config.SetUserParameters(new List<SqlParameter> { new SqlParameter("@static", 0) });
            config.AdditionalSettings.Add("userdequeue", "AND Static = 1");

            // Now set factory
            var factoryParams = new List<SqlParameter> { new SqlParameter("@factory", 99) };
        ...
        public void SetUserParametersAndClause_Factory_Overwrites_PreviousFactory()
        {
            ...
            config.SetUserParametersAndClause(
                () => new List<SqlParameter> { new SqlParameter("@first", 1) },
                () => "first");

            config.SetUserParametersAndClause(
                () => new List<SqlParameter> { new SqlParameter("@second", 2) },
                () => "second");

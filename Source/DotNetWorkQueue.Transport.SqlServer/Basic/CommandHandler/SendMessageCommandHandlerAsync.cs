...
namespace DotNetWorkQueue.Transport.SqlServer.Basic.CommandHandler
{
    ...
    internal class SendMessageCommandHandlerAsync : ICommandHandlerWithOutputAsync<SendMessageCommand, long>
    {
        ...
        private readonly RelationalDatabase.IQueryHandler<DoesJobExistQuery<SqlConnection, SqlTransaction>, QueueStatuses> _jobExistsHandler;
        ...
        public SendMessageCommandHandlerAsync(ITableNameHelper tableNameHelper,
            ...
            ICommandHandler<SetJobLastKnownEventCommand<SqlConnection, SqlTransaction>> sendJobStatus, RelationalDatabase.IQueryHandler<DoesJobExistQuery<SqlConnection, SqlTransaction>, QueueStatuses> jobExistsHandler, 
                            ...
                            if (id > 0)
                            {
                                ...
                                await
                                    CreateMetaDataRecordAsync(commandSend.MessageData.GetDelay(), expiration, connection,
                                        id,
                                        commandSend.MessageToSend, commandSend.MessageData, trans).ConfigureAwait(false);
        ...
        private async Task CreateMetaDataRecordAsync(TimeSpan? delay, TimeSpan expiration, SqlConnection connection, long id,
            IMessage message, IAdditionalMessageData data, SqlTransaction trans)
        {
            using (var command = connection.CreateCommand())
            {

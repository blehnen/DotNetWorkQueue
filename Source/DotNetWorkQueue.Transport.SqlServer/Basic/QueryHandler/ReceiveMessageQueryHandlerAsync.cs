...
namespace DotNetWorkQueue.Transport.SqlServer.Basic.QueryHandler
{
    ...
    internal class ReceiveMessageQueryHandlerAsync : IQueryHandler<ReceiveMessageQueryAsync, Task<IReceivedMessageInternal>>
    {
        ...
        public ReceiveMessageQueryHandlerAsync(ISqlServerMessageQueueTransportOptionsFactory optionsFactory,
                ...
                using (var reader = await selectCommand.ExecuteReaderAsync().ConfigureAwait(false))
                {
                    ...
                    catch (Exception error)
                    {
                        ...
                        throw new PoisonMessageException(
                            "An error has occurred trying to re-assemble a message de-queued from the SQL server", error, 
                            new SqlServerMessageQueueId(id), new SqlServerMessageQueueCorrelationId(correlationId),
